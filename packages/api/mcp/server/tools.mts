...
import { getCodeCell, SessionNotFoundError, CellNotFoundError } from './session-accessor.mjs';
import { executeAndCapture } from '../../exec.mjs';
...
async function createTaskForTool<T>(
  toolName: string,
  operation: () => Promise<T>,
): Promise<{ content: Array<{ type: 'text'; text: string }>; structuredContent: { success: boolean; taskId: string } }> {
  ...
  return {
    content: [{ type: 'text' as const, text: `Task ${task.taskId} created for ${toolName}` }],
    structuredContent: {
      success: true,
      taskId: task.taskId,
    },
...
function notImplemented(toolName: string): { content: Array<{ type: 'text'; text: string }>; structuredContent: { success: boolean; error: string } } {
...
export function registerNotebookTools(server: McpServer): void {
  // Note: execution hints removed as they're not part of MCP SDK tool options type
  ...
  server.registerTool(
    ...
    async (input, extra) => {
      const progressToken = (extra as { progressToken?: ProgressToken } | undefined)?.progressToken;
      ...
      return {
        content: [{ type: 'text' as const, text: `Notebook created: ${session.title}` }],
        structuredContent: {
          success: true,
  ...
  server.registerTool(
    ...
    async (input) => {
      ...
      return {
        content: [{ type: 'text' as const, text: 'Listed 0 notebooks' }],
        structuredContent: {
          notebooks: [],
          total: 0,
          limit: input.limit,
  ...
  server.registerTool(
    ...
    async (input) => {
      ...
      return {
        content: [{ type: 'text' as const, text: `Created ${input.type} cell: ${cell.id}` }],
        structuredContent: {
          success: true,
          cell,
        },
  ...
  server.registerTool(
    ...
    async (input) => {
      ...
      return {
        content: [{ type: 'text' as const, text: `Updated cell: ${input.cellId}` }],
        structuredContent: {
          success: true,
          cell,
        },
  ...
  server.registerTool(
    ...
    async (input) => {
      ...
      return {
        content: [{ type: 'text' as const, text: `Deleted cell: ${input.cellId}` }],
        structuredContent: {
          success: true,
  ...
  server.registerTool(
    ...
    async (input) => {
      ...
      return {
        content: [{ type: 'text' as const, text: `Moved cell ${input.cellId} to position ${input.newIndex}` }],
        structuredContent: {
          success: true,
  ...
  server.registerTool(
    ...
    async (input, extra) => {
      ...
      return createTaskForTool('cell_execute', async () => {
        startProgress(progressToken, 'Loading cell');

        try {
          // Load session and validate cell is a code cell
          const { session, cell } = await getCodeCell(input.sessionId, input.cellId);

          startProgress(progressToken, 'Running cell');

          // Build the entry path: session directory + src/ + cell filename
          const entryPath = Path.join(session.dir, 'src', cell.filename);
          ...
          const result = await executeAndCapture({
            ...
            maxOutputBytes: 1_048_576, // 1MB max output
          });
          ...
          if (error instanceof SessionNotFoundError) {
            logger.warning({ tool: 'cell_execute', error: 'Session not found', sessionId: input.sessionId });
            return {
              success: false,
          ...
          if (error instanceof CellNotFoundError) {
            logger.warning({ tool: 'cell_execute', error: 'Cell not found', sessionId: input.sessionId, cellId: input.cellId });
            return {
              success: false,
              error: `Cell not found: ${input.cellId} (must be a code cell)`,
              stdout: '',
              stderr: '',
              exitCode: null,
            };
          ...
          logger.error({ tool: 'cell_execute', error: errorMessage, sessionId: input.sessionId, cellId: input.cellId });
          return {
            success: false,
            error: errorMessage,
            stdout: '',
  ...
  server.registerTool(
    ...
    async (input) => {
      ...
      return {
        content: [{ type: 'text' as const, text: `Stop request for cell ${input.cellId}` }],
        structuredContent: {
          success: true,
